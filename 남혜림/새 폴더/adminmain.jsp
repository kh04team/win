...
<tiles:insertAttribute name="header"/>
  ...
  /* 		$("#comment").click(function () {
  			...
  			$('form[name=commentform]').attr({action:'qna_comment.action', method:'post'}).submit();
	...
	/* $("#comment").click(function () {
		...
		<s:if test="commentParentText == null">
			alert("내용을 입력해주세요.");
			$("#commentParentText").focus();
			return;
		</s:if>
		...
		{
			$('form[name=commentform]').attr({action:'/icase/qna_comment.action', method:'post'}).submit();
...
/*	
	function writeSave(){
		if(document.comment.value==null){
			alert ("내용을 입력해주세요.");
			document.comment.com.focus();
			return false;
		}
		
 		else(document.comment.valus="com"){
			alert("재작성 하시겠습니까? 이전 내용은 삭제됩니다.");
			document.comment.com.focus();
			return false;
		} 
 	} */
