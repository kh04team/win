...
public class goodsSettleForm extends ActionSupport {
	...
	private member_bin paramClass3 = new member_bin();
	private basket_bin resultClass = new basket_bin();
	private order_bin resultClass2 = new order_bin();
	private member_bin resultMember = new member_bin();
	...
	public String execute() throws Exception {
		...
		paramClass3 = new member_bin();
		paramClass3.setId((String)ActionContext.getContext().getSession().get("id"));
				
		resultMember = (member_bin) sqlMapper.queryForObject("member.selectMemberOne", paramClass3);				
		
		return SUCCESS;

	}

	public member_bin getResultMember() {
		return resultMember;
	}

	public void setResultMember(member_bin resultMember) {
	...
	public String orderSettle() throws Exception {
		...
		if(getSettle_type().equals("무통장"))
		{
			paramClass2.setOrder_status("결제대기");
		}
		else
		{
			paramClass2.setOrder_status("결제완료");			
		}
